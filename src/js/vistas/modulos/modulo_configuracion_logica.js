...
export async function iniciarModuloConfiguracion() {
    ...
    try {
        ...
        uidUsuarioActual = sesion.user.id;
...
const renderizarDatosPerfil = (u) => {
    ...
    if (u.organizaciones) {
        ...
        if (document.getElementById('correoClinica')) document.getElementById('correoClinica').value = u.organizaciones.correo_contacto || "";
        if (document.getElementById('sitioWebClinica')) document.getElementById('sitioWebClinica').value = u.organizaciones.sitio_web || "";
    }
    ...
    renderizarHorariosUI(u.horario_atencion);
};
...
const renderizarHorariosUI = (horarioData) => {
    if (!horarioData) return; // Deja los defaults del HTML

    const diasUI = ['Lunes', 'Martes', 'Miercoles', 'Jueves', 'Viernes', 'Sabado', 'Domingo'];
    
    // Adaptador de formato del Onboarding (ej. { dias: ['Lun', 'Mar'], apertura: '09:00', cierre: '18:00' })
    if (horarioData && typeof horarioData === 'object' && !Array.isArray(horarioData) && horarioData.dias) {
        const mapDiasCortos = { 'Lun':'Lunes', 'Mar':'Martes', 'Mie':'Miercoles', 'Jue':'Jueves', 'Vie':'Viernes', 'Sab':'Sabado', 'Dom':'Domingo' };
        const diasSeleccionados = horarioData.dias.map(d => mapDiasCortos[d]);
        
        diasUI.forEach(dia => {
            const abierto = diasSeleccionados.includes(dia);
            ...
            if (chk) chk.checked = abierto;
            if (ini) { ini.value = abierto ? horarioData.apertura : ''; ini.disabled = !abierto; }
            if (fin) { fin.value = abierto ? horarioData.cierre : ''; fin.disabled = !abierto; }
            if (lbl) {
                lbl.innerText = abierto ? "Abierto" : "Cerrado";
                lbl.style.color = abierto ? "var(--cobalto)" : "#E74C3C";
            }
        });
        return;
    }

    // Formato JSON Estructurado Final (Arreglo de objetos)
    if (Array.isArray(horarioData)) {
        horarioData.forEach(item => {
            const dia = item.dia; // Ej: 'Lunes'
            const chk = document.getElementById(`chk${dia}`);
            const ini = document.getElementById(`ini-${dia}`);
            const fin = document.getElementById(`fin-${dia}`);
            const lbl = document.getElementById(`estado${dia}`);

            if (chk) chk.checked = item.abierto;
            if (ini) { ini.value = item.abierto ? item.apertura : ''; ini.disabled = !item.abierto; }
            if (fin) { fin.value = item.abierto ? item.cierre : ''; fin.disabled = !item.abierto; }
            if (lbl) {
                lbl.innerText = item.abierto ? "Abierto" : "Cerrado";
                lbl.style.color = item.abierto ? "var(--cobalto)" : "#E74C3C";
            }
        });
    }
};
