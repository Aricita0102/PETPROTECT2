...
function configurarGuardadoECOP() {
    ...
    formConsulta?.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            // 3. Preparar Checkout (Cargos económicos para enviar a caja)
            const cargosParaCheckout = [];
            cargosParaCheckout.push({
                id: 'srv-consulta-base',
                categoria: 'cat-clinica',
                nombre: 'Consulta Médica General',
                precio: 500, 
                default: true
            });

            listaFarmacosManuales.forEach((farmaco, i) => {
                cargosParaCheckout.push({
                    id: `farmaco-manual-${i}`,
                    categoria: 'cat-farmacia',
                    nombre: farmaco.nombre_manual,
                    precio: 150 
                });
            });
