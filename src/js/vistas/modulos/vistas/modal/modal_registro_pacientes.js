...
function configurarEventosDelModalInyectado() {
    ...
    refs.btnConsulta?.addEventListener('click', async (e) => {
          e.preventDefault();

          const sesion = JSON.parse(localStorage.getItem('sesion_activa_petprotect') || 'null');
    ...
    refs.btnExpediente?.addEventListener('click', (e) => {
        ...
        if (pacienteRegistradoId) {
            const id = pacienteRegistradoId;
            cerrarModal();
            cargarModulo('expediente-detalle', { paciente_id: id });
        }
    });
}
...
async function ejecutarGuardado(e, refs) {
    ...
    try {
        const { data: userData, error: authErr } = await conexionSupabase.auth.getUser();
        if (authErr || !userData.user) throw new Error("Sesión clínica no válida.");
        const userId = userData.user.id;

        const { data: perfil, error: perfilErr } = await conexionSupabase
            .from('perfiles')
            .select('organizacion_id, sucursal_id')
            .eq('id', userId)
            .single();
        if (perfilErr) throw new Error("No se pudo resolver la clínica de origen.");
