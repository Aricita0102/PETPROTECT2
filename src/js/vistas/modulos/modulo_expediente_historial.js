...
import { iniciarTourExpediente, iniciarTourExpedienteSiEsPrimeraVez } from '../../tour/modulo_expedientes_tour.js';
...
export async function inicializarExpedienteHistorial() {
    ...
    const btnGuiaHistorial = document.getElementById('btn-guia-historial');
    if (btnGuiaHistorial) btnGuiaHistorial.addEventListener('click', iniciarTourExpediente);
    
    iniciarTourExpedienteSiEsPrimeraVez();
    ...
    // 4. Si tenemos los identificadores, lanzamos la carga paralela
    if (idPacienteActual) {
        await inicializarExpedienteHistorialParalelo(idPacienteActual);
    } else {
        console.error('[EXPEDIENTE] Error crítico: No se definió el paciente activo en sessionStorage.');
    }
}

// ==========================================================================
// 🚀 MOTOR DE CARGA PARALELA (N+1 SOLVED)
// ==========================================================================
async function inicializarExpedienteHistorialParalelo(idPacienteActual) {
    try {
        console.log("⏳ [PERFORMANCE] Iniciando carga paralela de expediente...");
        const inicioFetch = performance.now();

        // El Singleton ya maneja la identidad, nos saltamos auth.getUser() y from('perfiles')
        const sesion = await obtenerSesionActiva();
        if (!sesion) throw new Error("No hay sesión activa para el expediente.");

        // 🚀 PARALELIZACIÓN DE RED: 3 Peticiones simultáneas en lugar de cascada
        const [resPaciente, resConsultas, resEstudios] = await Promise.all([
            // 1. Datos de Identidad y Tutor
            conexionSupabase.from('pacientes')
            ...
            conexionSupabase.from('consultas')
            ...
            // 3. Estudios Clínicos y sus Archivos
            conexionSupabase.from('estudios_clinicos')
        ...
        console.log(`✅ [PERFORMANCE] Datos de expediente descargados en ${(finFetch - inicioFetch).toFixed(2)}ms`);

        const pacienteData = resPaciente.data;
        const consultasData = resConsultas.data || [];
        const estudiosData = resEstudios.data || [];

        // Se inyectan los datos PRE-CARGADOS a las funciones de renderizado, evitando nuevas llamadas a red
        await renderizarResumenExpediente(pacienteData, consultasData);
        configurarSubidaDeFoto();
        configurarMotorDeEdicion();
        configurarBotonesHuerfanos(pacienteData);
        configurarAccionesExpediente(pacienteData);
        ...
        renderizarTimelineHistorial(consultasData, estudiosData);
        
        // Iniciar carga de medicina preventiva
        cargarMedicinaPreventiva(idPacienteActual);

    } catch (error) {
        console.error("❌ Error crítico al cargar el expediente paralelo:", error);
    }
}
...
function configurarBotonesHuerfanos(pacienteData) {
    // 1. Añadir Consulta (existente)
    const btnNuevaConsulta = document.getElementById('btn-nueva-consulta-expediente');
    if (btnNuevaConsulta) {
        btnNuevaConsulta.addEventListener('click', (e) => {
            e.preventDefault();
    ...
    // 2. Agendar Cita (Resumen)
    const btnAgendarCita = document.getElementById('btn-agendar-cita-resumen');
    if (btnAgendarCita) {
        btnAgendarCita.addEventListener('click', (e) => {
            e.preventDefault();
            sessionStorage.setItem('agenda_paciente_id', idPacienteActual);
            cargarModulo('MODULO_AGENDA');
        });
    }

    // 3. Editar Perfil (Resumen)
    const btnEditarResumen = document.getElementById('btn-editar-perfil-resumen');
    if (btnEditarResumen) {
        btnEditarResumen.addEventListener('click', (e) => {
            e.preventDefault();
            if (window.abrirModalGlobal) window.abrirModalGlobal('identidad');
            else console.warn("abrirModalGlobal no disponible");
        });
    }

    // 4. Añadir Estudio (Historial)
    const btnEstudioHist = document.getElementById('btn-anadir-estudio-hist');
    if (btnEstudioHist) {
        btnEstudioHist.addEventListener('click', (e) => {
            e.preventDefault();
            const tabEstudios = document.querySelector('[data-target="seccion-estudios"]');
            if (tabEstudios) tabEstudios.click();
            setTimeout(() => {
                const btnOrden = document.getElementById('btn-registrar-nuevo-estudio');
                if (btnOrden) btnOrden.click();
            }, 100);
        });
    }

    // 5. Registrar Vacuna (Historial)
    const btnNuevaVacuna = document.getElementById('btn-nueva-vacuna-hist');
    if (btnNuevaVacuna) {
        btnNuevaVacuna.addEventListener('click', (e) => {
            e.preventDefault();
            abrirModalRegistroPreventivo();
        });
    }
}

// ==========================================================================
// 🖨️ ACCIONES DE EXPEDIENTE (IMPRIMIR, EXPORTAR, WHATSAPP)
// ==========================================================================
function configurarAccionesExpediente(pacienteData) {
    const btnImprimir = document.getElementById('btn-imprimir-expediente');
    ...
    if (btnExportar) {
        btnExportar.addEventListener('click', (e) => {
            e.preventDefault();
            // Utiliza el print del navegador para 'Guardar como PDF'
            window.print(); 
        });
    }
    ...
    if (btnWhatsapp) {
        btnWhatsapp.addEventListener('click', (e) => {
            e.preventDefault();
            if (!pacienteData || !pacienteData.clientes || !pacienteData.clientes.telefono) {
                if (typeof alertaCustom === 'function') {
                    alertaCustom("Atención", "No se encontró el teléfono del tutor registrado.", "warning");
                } else {
                    alert("No se encontró el teléfono del tutor registrado.");
                }
                return;
            }
            const tel = pacienteData.clientes.telefono.replace(/\D/g, ''); // Extraer solo números
            const nombreMascota = pacienteData.nombre || 'la mascota';
            const mensaje = `Hola, te contactamos de la clínica veterinaria para compartirte que el expediente de ${nombreMascota} ha sido actualizado. Quedamos a tu disposición para cualquier duda.`;
...
async function cargarEstudiosPaciente(datosPreCargados) {
    ...
    try {
        ...
        const tarjetas = estudios.map(e => {
            ...
            return `
            <div class="study-card" data-estudio-id="${e.id}" data-modalidad="${e.modalidad}" data-fecha="${e.fecha_estudio ? e.fecha_estudio : ''}">
        ...
        // Configurar filtros si no están configurados aún
        if (!window.filtrosEstudiosConfigurados) {
            configurarFiltrosEstudios();
            window.filtrosEstudiosConfigurados = true;
        }
...
// ==========================================================================
function configurarFiltrosEstudios() {
    const checkboxes = document.querySelectorAll('.filtro-estudio-cb');
    const inputInicio = document.getElementById('filtro-estudio-inicio');
    const inputFin = document.getElementById('filtro-estudio-fin');

    const aplicarFiltros = () => {
        // Obtener modalidades seleccionadas
        const modalidadesSeleccionadas = Array.from(checkboxes)
            .filter(cb => cb.checked)
            .map(cb => cb.value);

        // Obtener fechas
        const fechaInicio = inputInicio.value ? new Date(inputInicio.value + 'T00:00:00') : null;
        const fechaFin = inputFin.value ? new Date(inputFin.value + 'T23:59:59') : null;

        // Filtrar tarjetas
        const tarjetas = document.querySelectorAll('.study-card');
        tarjetas.forEach(tarjeta => {
            const modalidad = tarjeta.getAttribute('data-modalidad');
            const fechaAttr = tarjeta.getAttribute('data-fecha');
            
            // 1. Validar modalidad
            let mostrarModalidad = modalidadesSeleccionadas.includes(modalidad);
            if (!modalidadesSeleccionadas.includes(modalidad) && modalidadesSeleccionadas.length > 0) {
                // hide
            } else if (modalidadesSeleccionadas.length === 0) {
                mostrarModalidad = false; // si no hay ninguno checkeado, no mostrar nada
            } else {
                // Si es un tipo no listado en checkboxes, lo mostraremos por defecto para no perderlo
                const todasLasOpciones = Array.from(checkboxes).map(cb => cb.value);
                if (!todasLasOpciones.includes(modalidad)) {
                    mostrarModalidad = true;
                }
            }

            // 2. Validar fechas
            let mostrarFecha = true;
            if (fechaAttr && (fechaInicio || fechaFin)) {
                const fechaEstudio = new Date(fechaAttr + 'T12:00:00');
                if (fechaInicio && fechaEstudio < fechaInicio) mostrarFecha = false;
                if (fechaFin && fechaEstudio > fechaFin) mostrarFecha = false;
            }

            // Aplicar visibilidad
            if (mostrarModalidad && mostrarFecha) {
    ...
    if (inputInicio) inputInicio.addEventListener('change', aplicarFiltros);
    if (inputFin) inputFin.addEventListener('change', aplicarFiltros);
}
...
async function renderizarResumenExpediente(paciente, consultasData) {
    try {
...
// ==========================================================================
// 🛡️ MÓDULO: MEDICINA PREVENTIVA Y VACUNAS
// ==========================================================================
async function cargarMedicinaPreventiva(idPaciente) {
    const contenedor = document.getElementById('res-preventiva-container');
    if (!contenedor) return;
    ...
    try {
        ...
        // Si la tabla no existe o hay error (ej. PostgreSQL error code 42P01 "undefined_table")
        if (error) {
            console.warn('[EXPEDIENTE] Error o tabla pacientes_preventiva no existe:', error.message);
            contenedor.innerHTML = '<div style="text-align: center; color: var(--gris-texto-mutado); padding: 20px;">Módulo de prevención en configuración... (Ejecutar script SQL)</div>';
            return;
        }

        if (!data || data.length === 0) {
            contenedor.innerHTML = '<div style="text-align: center; color: var(--gris-texto-mutado); padding: 20px;">Sin registros preventivos</div>';
            return;
        }

        let html = '';
        data.forEach(item => {
            const esProximo = item.proxima_aplicacion && new Date(item.proxima_aplicacion) >= new Date();
            const colorTipo = item.tipo === 'vacuna' ? '#10b981' : (item.tipo === 'desparasitacion' ? '#f59e0b' : '#3b82f6');
            const iconTipo = item.tipo === 'vacuna' ? 'vaccines' : (item.tipo === 'desparasitacion' ? 'bug_report' : 'event_note');

            html += `
                <div style="background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 8px; padding: 12px; display: flex; align-items: flex-start; gap: 12px;">
                    <div style="background: ${colorTipo}20; color: ${colorTipo}; padding: 8px; border-radius: 50%; display: flex;">
                        <span class="material-symbols-rounded" style="font-size: 20px;">${iconTipo}</span>
                    </div>
                    <div style="flex: 1;">
                        <h4 style="margin: 0; font-size: 14px; font-weight: 700; color: #1e293b;">${item.nombre_procedimiento}</h4>
                        <p style="margin: 4px 0 0; font-size: 12px; color: #64748b;">Aplicación: ${item.fecha_aplicacion}</p>
                        ${item.proxima_aplicacion ? `<p style="margin: 4px 0 0; font-size: 12px; font-weight: 600; color: ${esProximo ? '#f59e0b' : '#ef4444'};">Próxima: ${item.proxima_aplicacion}</p>` : ''}
                    </div>
                </div>
            `;
        });
        ...
        console.error("[PREVENTIVA] Error al cargar:", err);
    }
}

async function abrirModalRegistroPreventivo() {
    if (typeof Swal === 'undefined') {
        alertaCustom('Error: SweetAlert2 no está cargado.', '', 'error');
        return;
    }

    const { value: formValues } = await Swal.fire({
        title: 'Registrar Procedimiento Preventivo',
        html: `
            <div style="text-align: left; display: flex; flex-direction: column; gap: 10px;">
                <label style="font-weight: 600; font-size: 13px;">Tipo de Procedimiento</label>
                <select id="swal-prev-tipo" class="swal2-input" style="margin:0; width: 100%;">
                    <option value="vacuna">Vacunación</option>
                    <option value="desparasitacion">Desparasitación</option>
                    <option value="recordatorio">Recordatorio General</option>
                </select>
                
                <label style="font-weight: 600; font-size: 13px; margin-top: 10px;">Nombre (Ej. Sextuple, Bravecto)</label>
                <input id="swal-prev-nombre" class="swal2-input" style="margin:0; width: 100%; box-sizing: border-box;" placeholder="Nombre del procedimiento">
                
                <div style="display: flex; gap: 10px; margin-top: 10px;">
                    <div style="flex: 1;">
                        <label style="font-weight: 600; font-size: 13px;">Fecha Aplicación</label>
                        <input id="swal-prev-fecha" type="date" class="swal2-input" style="margin:0; width: 100%;" value="${new Date().toISOString().split('T')[0]}">
                    </div>
                    <div style="flex: 1;">
                        <label style="font-weight: 600; font-size: 13px;">Próxima Aplicación</label>
                        <input id="swal-prev-proxima" type="date" class="swal2-input" style="margin:0; width: 100%;">
                    </div>
                </div>
                
                <label style="font-weight: 600; font-size: 13px; margin-top: 10px;">Notas Adicionales (Lote, Peso, etc.)</label>
                <textarea id="swal-prev-notas" class="swal2-textarea" style="margin:0; width: 100%; box-sizing: border-box; height: 60px;" placeholder="Opcional..."></textarea>
            </div>
        `,
        focusConfirm: false,
        ...
        confirmButtonColor: '#032F40',
        preConfirm: () => {
            const tipo = document.getElementById('swal-prev-tipo').value;
            const nombre = document.getElementById('swal-prev-nombre').value.trim();
            const fecha = document.getElementById('swal-prev-fecha').value;
            const proxima = document.getElementById('swal-prev-proxima').value;
            const notas = document.getElementById('swal-prev-notas').value.trim();

            if (!nombre || !fecha) {
                Swal.showValidationMessage('El nombre y la fecha de aplicación son obligatorios');
                return false;
            }

            return { tipo, nombre, fecha, proxima: proxima || null, notas };
        }
    });
    ...
    if (formValues) {
        try {
            ...
            const payload = {
                ...
                nombre_procedimiento: formValues.nombre,
                fecha_aplicacion: formValues.fecha,
                proxima_aplicacion: formValues.proxima,
                notas: formValues.notas
            };

            const { error } = await conexionSupabase.from('pacientes_preventiva').insert([payload]);
            if (error) throw error;

            Swal.fire('¡Guardado!', 'El registro preventivo ha sido añadido.', 'success');
            
            // Recargar la tabla
            cargarMedicinaPreventiva(idPacienteActual);

        } catch (err) {
            console.error('[PREVENTIVA] Error guardando registro:', err);
            Swal.fire('Error', 'No se pudo guardar. Verifica que la tabla pacientes_preventiva exista en Supabase.', 'error');
        }
    }
}
