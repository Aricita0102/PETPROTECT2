import{n as e}from"./conexion-DiH-JvMT.js";import{n as t}from"./sesion_store-BSrAgZd7.js";import{t as n}from"./PRINCIPAL-BYI241S5.js";var r=`<aside id="contenedorRegistro" class="modal-backdrop-premium modal-lateral-backdrop" aria-hidden="true">\r
    <style>\r
...
</aside>`,i=!1,a=null,o={Lagomorfo:[`Cabeza de León`,`Belier`,`Angora`,`Rex`,`Mestizo`],Roedor:[`Hámster Sirio`,`Hámster Ruso`,`Cobaya`,`Chinchilla`,`Rata Dumbo`,`Mestizo`],Mustelido:[`Hurón Sable`,`Hurón Albino`,`Hurón Panda`],Porcino:[`Mini Pig`,`Cerdo Vietnamita`],Erinaceomorfo:[`Erizo Pigmeo Africano`,`Erizo Europeo`],Marsupial:[`Petauro del Azúcar`],Ave_Psitaciforme:[`Periquito`,`Ninfa / Carolina`,`Guacamayo`,`Loro Gris`,`Agapornis`,`Cacatúa`],Ave_Paseriforme:[`Canario`,`Jilguero`,`Diamante Mandarín`],Ave_Corral:[`Gallina`,`Pato`,`Pavo`],Ave_Rapaz:[`Halcón`,`Búho`,`Cernícalo`],Reptil_Chelonio:[`Tortuga de Orejas Rojas`,`Tortuga Sulcata`,`Tortuga Terrestre`],Reptil_Squamata:[`Iguana Verde`,`Dragón Barbudo`,`Camaleón`,`Gecko Leopardo`],Reptil_Serpiente:[`Pitón Bola`,`Boa Constrictor`,`Falsa Coral`],Anfibio:[`Ajolote`,`Rana Arborícola`,`Sapo`],Equino:[`Cuarto de Milla`,`Pura Sangre`,`Árabe`,`Mestizo`],Bovino:[`Holstein`,`Angus`,`Cebú`],Caprino:[`Boer`,`Alpina`],Ovino:[`Dorper`,`Suffolk`,`Pelibuey`],Otro:[]};async function s(){console.log(`[MODAL TRIAGE] Preparando inyección del formulario express...`);let e=document.getElementById(`contenedor-modal-dinamico`);if(!e){console.error(`❌ ERROR: No existe #contenedor-modal-dinamico en la vista principal.`);return}!i&&!document.getElementById(`contenedorRegistro`)&&(e.innerHTML=r,i=!0,await new Promise(e=>setTimeout(e,0)),c());let t=document.getElementById(`contenedorRegistro`);t&&(t.classList.add(`visible`),t.setAttribute(`aria-hidden`,`false`),document.body.style.overflow=`hidden`)}function c(){let e={modal:document.getElementById(`contenedorRegistro`),form:document.getElementById(`formRegistroExpress`),btnCerrar:document.getElementById(`btnCerrarRegistro`),regEspecie:document.getElementById(`exp_especie`),btnGuardar:document.getElementById(`btn_guardar_expediente`),btnConsulta:document.getElementById(`btn_iniciar_consulta`),btnExpediente:document.getElementById(`btn_completar_expediente`)},t=()=>{e.modal.classList.remove(`visible`),e.modal.setAttribute(`aria-hidden`,`true`),document.body.style.overflow=`auto`,e.form&&e.form.reset(),l(null),a=null,e.btnGuardar&&(e.btnGuardar.style.display=`flex`),e.btnExpediente&&(e.btnExpediente.style.display=`none`),e.btnConsulta&&(e.btnConsulta.style.display=`none`)};e.btnCerrar?.addEventListener(`click`,t),e.modal?.addEventListener(`click`,n=>{n.target===e.modal&&t()}),e.regEspecie?.addEventListener(`change`,e=>{l(e.target.value),u(e.target.value)}),e.btnGuardar?.addEventListener(`click`,t=>d(t,e)),e.btnConsulta?.addEventListener(`click`,e=>{if(e.preventDefault(),a){let e=a;t(),n(`MODULO_VETERINARIO_CONSULTA`,{paciente_id:e})}}),e.btnExpediente?.addEventListener(`click`,e=>{if(e.preventDefault(),a){let e=a;t(),n(`MODULO_EXPEDIENTES_HISTORIAL`,{paciente_id:e})}})}function l(e){let t=document.getElementById(`exp_especie`),n=document.getElementById(`exp_especie_otra`);n||(n=document.createElement(`input`),n.type=`text`,n.id=`exp_especie_otra`,n.name=`especie_otra`,n.placeholder=`Escribe la especie (Ej. Hurón, Tortuga...)`,n.style.marginTop=`10px`,n.style.display=`none`,n.classList.add(`form-control`),t.parentNode.insertBefore(n,t.nextSibling)),e===`Otro`?(n.style.display=`block`,n.required=!0,n.focus()):(n.style.display=`none`,n.required=!1,n.value=``)}async function u(e){let t=document.getElementById(`exp_raza`),n=document.getElementById(`lista-razas`);if(!(!t||!n)){if(t.value=``,n.innerHTML=``,e===`Otro`){t.placeholder=`Escribe la raza (o 'Desconocida')...`,t.disabled=!1;return}t.placeholder=`Sincronizando catálogo...`,t.disabled=!0;try{let r=[];if(e===`canino`){let e=await(await fetch(`https://dog.ceo/api/breeds/list/all`)).json();for(let[t,n]of Object.entries(e.message))n.length>0?n.forEach(e=>r.push(`${t} ${e}`)):r.push(t);r.push(`Mestizo`)}else e===`felino`?((await(await fetch(`https://api.thecatapi.com/v1/breeds`)).json()).forEach(e=>r.push(e.name)),r.push(`Mestizo / Común`)):r=o[e]||[`Desconocida`,`Mestizo`];r.forEach(e=>{let t=document.createElement(`option`);t.value=e.charAt(0).toUpperCase()+e.slice(1),n.appendChild(t)}),t.placeholder=`Escribe o selecciona la raza...`}catch(e){console.error(`[FALLO AL CARGAR RAZAS]`,e),t.placeholder=`Escribe la raza...`}finally{t.disabled=!1}}}async function d(n,r){if(n.preventDefault(),r.form&&!r.form.checkValidity()){r.form.reportValidity();return}let i=r.btnGuardar.innerHTML;r.btnGuardar.disabled=!0,r.btnGuardar.innerHTML=`<span class="material-symbols-rounded" style="animation: spin 1s linear infinite;">sync</span> Guardando...`;try{let n=await t();if(!n)throw Error(`Sesión clínica no válida.`);let i=n.user.id,o=n.perfil,s=document.getElementById(`exp_tutor_tel`)?.value.trim()||``;s=s.replace(/\D/g,``).substring(0,10);let c=document.getElementById(`exp_tutor_nombre`)?.value.trim(),l=null,{data:u,error:d}=await e.from(`clientes`).select(`id`).eq(`telefono`,s).eq(`organizacion_id`,o.organizacion_id).maybeSingle();if(d)throw d;if(u)l=u.id;else{let{data:t,error:n}=await e.from(`clientes`).insert([{organizacion_id:o.organizacion_id,sucursal_id:o.sucursal_id,nombre_completo:c,telefono:s,correo:`pendiente@express.com`,direccion:`Pendiente de actualización`,created_by:i}]).select().single();if(n)throw n;l=t.id}let f=document.getElementById(`exp_especie`)?.value;if(f===`Otro`){let e=document.getElementById(`exp_especie_otra`);f=e?e.value.trim():`Desconocida`}let p=document.getElementById(`exp_sexo_repro`)?.value||`indefinido`,m=document.getElementById(`exp_alergias`)?.value.trim()||`Sin alergias conocidas`,h=document.getElementById(`exp_temperamento`)?.value||`docil`,g=`MOTIVO: ${document.getElementById(`exp_motivo_consulta`)?.value.trim()||`Sin especificar`} | MANEJO: ${h}`,_=parseInt(document.getElementById(`exp_edad_anios`)?.value)||0,v=parseInt(document.getElementById(`exp_edad_meses`)?.value)||0,y=null;if(_>0||v>0){let e=new Date;e.setFullYear(e.getFullYear()-_),e.setMonth(e.getMonth()-v),y=e.toISOString().split(`T`)[0]}let{data:b,error:x}=await e.from(`pacientes`).insert([{cliente_id:l,organizacion_id:o.organizacion_id,nombre:document.getElementById(`exp_nombre_paciente`)?.value.trim(),especie:f,raza:document.getElementById(`exp_raza`)?.value.trim()||`Mestizo`,sexo:p,peso_actual:parseFloat(document.getElementById(`exp_peso`)?.value)||0,fecha_nacimiento:y,alergias:m,notas_comportamiento:g,foto_url:null,estatus:`activo`,created_by:i}]).select().single();if(x)throw x;a=b.id,r.btnGuardar.style.display=`none`,r.btnExpediente&&(r.btnExpediente.style.display=`flex`),r.btnConsulta&&(r.btnConsulta.style.display=`flex`)}catch(e){console.error(`Error en registro:`,e),alert(`Error de integridad: `+e.message),r.btnGuardar.disabled=!1,r.btnGuardar.innerHTML=i}}export{s as inyectarYMostrarModal};