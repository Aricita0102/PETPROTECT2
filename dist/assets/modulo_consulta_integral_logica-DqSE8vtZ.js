import{i as e,n as t}from"./conexion-DiH-JvMT.js";import{n}from"./ui_alertas-rGPCOazm.js";function r(){return`
  :root{
    --cobalto:#032F40;
    --naranja:#F27405;
    --blanco-puro:#FFFFFF;
  ...
  @media print{
    ...
    `}function i(e){let t=e.logo_url?`<img src="${e.logo_url}" alt="Logo">`:`
    ...
    `}function a(e){let t=window.open(``,`_blank`,`width=850,height=1000`);if(!t){alert(`Por favor, permite las ventanas emergentes (pop-ups) para imprimir la receta.`);return}let n=r(),a=i(e),o=t.document;o.open(),o.write(`
        <!DOCTYPE html>
        <html lang="es">
        <head>
            <meta charset="UTF-8">
    ...
    `),o.close()}var o={perfilMedico:null,pacienteActual:null,idCitaVinculada:null,intervaloReloj:null,intervaloCronometro:null,segundosTranscurridos:0},s={canino:{temp:{min:37.5,max:39.2,letalMin:34,letalMax:42.5},fc:{min:60,max:140,letalMin:30,letalMax:250}},felino:{temp:{min:38,max:39.5,letalMin:35,letalMax:43},fc:{min:140,max:220,letalMin:80,letalMax:300}}};async function c(){console.info(`[CLÍNICA] Despertando módulo de Consulta Integral...`),await h(),l(),f(),g(),v(),y(),x(),_(),d(),typeof suscribirAAlertasMedicas==`function`&&suscribirAAlertasMedicas();let e=sessionStorage.getItem(`iniciarConsultaDirecta`),t=sessionStorage.getItem(`idPacienteActivo`);if(e===`true`&&t)sessionStorage.removeItem(`iniciarConsultaDirecta`),await p(t),u();else if(window._agendaDatos){let{pacienteId:e,citaId:t}=window._agendaDatos;o.citaActivaId=t,window._agendaDatos=null,await p(e),u()}}function l(){let e=document.getElementById(`txt-fecha-hora-actual`);e&&(o.intervaloReloj=setInterval(()=>{e.textContent=new Date().toLocaleDateString(`es-MX`,{year:`numeric`,month:`short`,day:`numeric`,hour:`2-digit`,minute:`2-digit`,second:`2-digit`})},1e3))}function u(){let e=document.getElementById(`contenedor-cronometro`),t=document.getElementById(`txt-cronometro-consulta`);!e||!t||(e.hidden=!1,o.segundosTranscurridos=0,clearInterval(o.intervaloCronometro),o.intervaloCronometro=setInterval(()=>{o.segundosTranscurridos++,t.textContent=`${Math.floor(o.segundosTranscurridos/60).toString().padStart(2,`0`)}:${(o.segundosTranscurridos%60).toString().padStart(2,`0`)}`},1e3))}function d(){let e=sessionStorage.getItem(`idCitaActiva`);if(!e)return;let t=[`in-peso`,`in-temp`,`in-fc`,`in-fr`,`in-mucosas`,`in-tllc`,`in-dolor`,`in-anamnesis`,`in-diagnostico`,`in-indicaciones-receta`],n=()=>{let n={};t.forEach(e=>{let t=document.getElementById(e);t&&(n[e]=t.value)}),localStorage.setItem(`borrador_consulta_`+e,JSON.stringify(n))};t.forEach(e=>{let t=document.getElementById(e);t&&(t.addEventListener(`input`,n),t.addEventListener(`change`,n))});let r=localStorage.getItem(`borrador_consulta_`+e);if(r)try{let n=JSON.parse(r),i=!1;t.forEach(e=>{let t=document.getElementById(e);t&&n[e]!==void 0&&n[e]!==``&&(t.value=n[e],i=!0)}),i&&console.info(`[BORRADOR] Datos recuperados exitosamente para la cita `+e)}catch(e){console.warn(`[BORRADOR] No se pudo parsear el borrador`,e)}}function f(){let e=document.getElementById(`input-busqueda-paciente`),n=document.getElementById(`dropdownResultadosConsulta`);if(!e||!n)return;let r=null;e.addEventListener(`input`,e=>{let t=e.target.value.trim();if(clearTimeout(r),t.length<2){n.style.display=`none`;return}r=setTimeout(()=>i(t),400)}),document.addEventListener(`click`,t=>{!e.contains(t.target)&&!n.contains(t.target)&&(n.style.display=`none`)}),e.addEventListener(`focus`,()=>{e.value.trim().length>=2&&(n.style.display=`block`)});async function i(e){if(o.perfilMedico?.organizacionId){n.innerHTML=`<div class="resultado-pred-empty">Buscando...</div>`,n.style.display=`block`;try{let n=t.from(`pacientes`).select(`id, nombre, especie, raza, foto_url, clientes!inner(nombre_completo)`).eq(`organizacion_id`,o.perfilMedico.organizacionId).ilike(`nombre`,`%${e}%`).limit(5),r=t.from(`pacientes`).select(`id, nombre, especie, raza, foto_url, clientes!inner(nombre_completo)`).eq(`organizacion_id`,o.perfilMedico.organizacionId).ilike(`clientes.nombre_completo`,`%${e}%`).limit(5),[i,s]=await Promise.all([n,r]);if(i.error)throw i.error;if(s.error)throw s.error;let c=new Map;[...i.data||[],...s.data||[]].forEach(e=>{c.set(e.id,e)}),a(Array.from(c.values()).slice(0,6))}catch(e){console.error(`[CONSULTA] Error en buscador:`,e),n.innerHTML=`<div class="resultado-pred-empty" style="color:red;">Error de búsqueda</div>`}}}function a(t){if(t.length===0){n.innerHTML=`<div class="resultado-pred-empty">No se encontraron pacientes ni tutores.</div>`;return}n.innerHTML=``,t.forEach(t=>{let r=t.foto_url||`https://cdn-icons-png.flaticon.com/512/2809/2809865.png`,i=t.nombre||`Desconocido`,a=`${t.especie||`Mascota`} ${t.raza?`· `+t.raza:``}`,o=t.clientes?.nombre_completo||`Sin tutor registrado`,s=document.createElement(`div`);s.className=`resultado-pred-item`,s.innerHTML=`
            ...
            `,s.addEventListener(`click`,async()=>{n.style.display=`none`,e.value=``,await p(t.id),u()}),n.appendChild(s)})}}async function p(e){try{let n=document.getElementById(`txt-nombre-paciente`);n&&(n.textContent=`Cargando datos...`);let{data:r,error:i}=await t.from(`pacientes`).select(`*, clientes ( nombre_completo, telefono )`).eq(`id`,e).single();if(i)throw i;o.pacienteActual=r,document.getElementById(`txt-nombre-paciente`).textContent=r.nombre||`Desconocido`,document.getElementById(`txt-especie-raza-paciente`).textContent=`${r.especie} - ${r.raza||`No especificada`}`;let a=document.getElementById(`in-especie-oculta`);a&&(a.value=r.especie),document.getElementById(`txt-id-oficial`).textContent=r.chip_id||r.siniiga_tatuaje||`Sin ID Oficial`,document.getElementById(`txt-edad-paciente`).textContent=m(r.fecha_nacimiento),document.getElementById(`txt-dueno-paciente`).textContent=r.clientes?.nombre_completo||`Sin tutor`;let s=document.getElementById(`txt-tel-paciente`);s&&(s.textContent=r.clientes?.telefono||`--`);let c=document.getElementById(`txt-estado-reproductivo`);c&&(c.textContent=r.esta_esterilizado?`Esterilizado`:`Entero`);let l=document.getElementById(`txtComportamiento`);l&&(l.textContent=r.temperamento||`No evaluado`);let u=document.getElementById(`img-avatar-paciente`);u&&(r.foto_url?(u.style.backgroundImage=`url('${r.foto_url}')`,u.style.backgroundSize=`cover`,u.style.backgroundPosition=`center`,u.textContent=``):(u.style.backgroundImage=`none`,u.style.backgroundColor=`var(--gris-fondo-sutil)`,u.textContent=r.nombre?r.nombre.charAt(0).toUpperCase():`?`));let d=document.getElementById(`alerta-alergias`),f=document.getElementById(`txt-alergias-paciente`);r.alergias&&r.alergias.toLowerCase()!==`sin alergias conocidas`?(f&&(f.textContent=r.alergias),d&&(d.hidden=!1)):d&&(d.hidden=!0)}catch(e){console.error(`[CONSULTA INTEGRAL] Error al recuperar la información del paciente derivado:`,e),alert(`Ocurrió un error al trasladar los datos del paciente a la consulta.`);let t=document.getElementById(`txt-nombre-paciente`);t&&(t.textContent=`Selecciona Paciente`)}}function m(e){if(!e)return`--`;let t=new Date,n=new Date(e+`T12:00:00Z`),r=t.getFullYear()-n.getFullYear(),i=t.getMonth()-n.getMonth();return(i<0||i===0&&t.getDate()<n.getDate())&&r--,r>0?`${r} años`:`Meses`}async function h(){let n=await e();if(!n)return window.location.assign(`/LOGIN.html`);try{let{data:e,error:r}=await t.from(`perfiles`).select(`
            ...
            `).eq(`id`,n.id).single();if(r)throw r;o.perfilMedico={id:e.id,organizacionId:e.organizacion_id,sucursalId:e.sucursal_id,nombre:e.nombre_completo||`Dr. Sin Nombre`,cedula:e.cedula_profesional,firma_url:e.firma_url,clinica:e.organizaciones||{},sucursal:e.sucursales||{}}}catch(e){console.error(`[SEGURIDAD] Fallo de integridad en identidad:`,e),window.location.assign(`/LOGIN.html`)}}function g(){[`in-temp`,`in-fc`].forEach(e=>{let t=document.getElementById(e);t&&t.addEventListener(`blur`,t=>{if(!o.pacienteActual)return;let n=parseFloat(t.target.value),r=o.pacienteActual.especie.toLowerCase(),i=e===`in-temp`?`temp`:`fc`,a=s[r]?.[i];if(!a||isNaN(n))return;let c=document.getElementById(`error-${i}`);n<=a.letalMin||n>=a.letalMax||n<a.min||n>a.max?(t.target.classList.add(`error-active`),t.target.style.borderColor=`#F27405`,c.textContent=`Fuera de rango normal.`,c.style.color=`#F27405`,c.style.display=`block`):(t.target.classList.remove(`error-active`),t.target.style.borderColor=``,c.style.display=`none`)})})}function _(){document.querySelectorAll(`.control-pills-naranja`).forEach(e=>{let t=e.querySelectorAll(`.pill-btn-naranja`);t.forEach(e=>{e.addEventListener(`click`,e=>{t.forEach(e=>{e.classList.remove(`activo`),e.setAttribute(`aria-checked`,`false`)});let n=e.currentTarget;n.classList.add(`activo`),n.setAttribute(`aria-checked`,`true`)})})})}function v(){let e=document.getElementById(`btn-abrir-modal-farmaco`),t=document.getElementById(`tbody-receta-farmacos`);!e||!t||(e.textContent=`+ Añadir Fila a Receta`,e.addEventListener(`click`,()=>{let e=document.createElement(`tr`);e.className=`fila-receta-manual`,e.innerHTML=`
        ...
        `,t.appendChild(e)}))}function y(){let e=document.getElementById(`btn-ver-expediente`),t=document.querySelector(`.rejilla-bento-consulta`),n=document.getElementById(`panel-expediente`),r=document.getElementById(`btn-cerrar-expediente`);e?.addEventListener(`click`,async()=>{if(!o.pacienteActual)return alert(`⚠️ SAFETY FIRST: Seleccione un paciente para ver su expediente.`);t.classList.contains(`expediente-abierto`)?(t.classList.remove(`expediente-abierto`),n&&n.setAttribute(`aria-hidden`,`true`)):(t.classList.add(`expediente-abierto`),n&&n.setAttribute(`aria-hidden`,`false`),await b(o.pacienteActual.id))}),r?.addEventListener(`click`,()=>{t.classList.remove(`expediente-abierto`),n&&n.setAttribute(`aria-hidden`,`true`)});let i=document.getElementById(`btn-limpiar-paciente`);i&&i.addEventListener(`click`,()=>{o.pacienteActual=null,document.getElementById(`txt-nombre-paciente`).textContent=`Selecciona Paciente`,document.getElementById(`txt-especie-raza-paciente`).textContent=`---`;let e=document.getElementById(`in-especie-oculta`);e&&(e.value=``),document.getElementById(`txt-id-oficial`).textContent=`---`,document.getElementById(`txt-edad-paciente`).textContent=`---`,document.getElementById(`txt-dueno-paciente`).textContent=`---`;let r=document.getElementById(`txt-tel-paciente`);r&&(r.textContent=`---`);let i=document.getElementById(`txt-estado-reproductivo`);i&&(i.textContent=`---`);let a=document.getElementById(`txtComportamiento`);a&&(a.textContent=`---`);let s=document.getElementById(`img-avatar-paciente`);s&&(s.style.backgroundImage=`none`,s.textContent=`?`),t.classList.remove(`expediente-abierto`),n&&n.setAttribute(`aria-hidden`,`true`),document.getElementById(`contenedor-buscador-consulta`).style.display=`block`,document.getElementById(`form-consulta-integral`).reset();let c=document.getElementById(`tbody-receta-farmacos`);c&&(c.innerHTML=``),listaFarmacosManuales=[],clearInterval(o.intervaloCronometro);let l=document.getElementById(`contenedor-cronometro`);l&&(l.hidden=!0)})}async function b(e){let n=document.getElementById(`lista-historial-paciente`);if(n){n.innerHTML=`<p>Cargando datos inmutables...</p>`;try{let{data:r,error:i}=await t.from(`consultas`).select(`id, created_at, diagnostico_presuntivo, frecuencia_cardiaca, temperatura, motivo_consulta`).eq(`paciente_id`,e).order(`created_at`,{ascending:!1}).limit(5);if(i)throw i;if(r.length===0){n.innerHTML=`<p class="texto-secundario text-center">No hay registros previos para este paciente.</p>`;return}n.innerHTML=r.map(e=>`
        ...
        `).join(``)}catch(e){console.error(`Error al cargar expediente:`,e),n.innerHTML=`<p class='error-msg'>Error al recuperar historial.</p>`}}}function x(){let e=document.getElementById(`form-consulta-medica`);e?.addEventListener(`submit`,async r=>{if(r.preventDefault(),!o.pacienteActual)return alert(`⚠️ Seleccione un paciente antes de generar la consulta.`);let i=[],s=!1;if(document.querySelectorAll(`.fila-receta-manual`).forEach(e=>{let t=e.querySelector(`.farmaco-nombre`).value.trim(),n=e.querySelector(`.farmaco-dosis`).value.trim(),r=e.querySelector(`.farmaco-frecuencia`).value.trim(),a=parseInt(e.querySelector(`.farmaco-tipo`).value);t&&((a===1||a===2)&&!o.perfilMedico.cedula&&(s=!0),i.push({farmaco_id:null,nombre_manual:t,dosis_mg_kg:0,dosis_total_ml_tabletas:0,via_admin:n,frecuencia:r,requiere_cuantificada:a===1||a===2}))}),s)return alert(`🛑 SEGURIDAD LEGAL: No cuenta con Cédula Profesional registrada para prescribir medicamentos controlados (Grupos I y II).`);let c=parseFloat(document.getElementById(`in-peso`).value),l=parseFloat(document.getElementById(`in-temp`).value),u=parseInt(document.getElementById(`in-fc`).value),d=parseInt(document.getElementById(`in-fr`).value),f=parseInt(document.getElementById(`in-dolor`).value)||0;if(isNaN(c)||isNaN(l)||isNaN(u)||isNaN(d))return alert(`⚠️ Por favor, rellena todas las constantes vitales con valores numéricos válidos.`);let p=document.getElementById(`btn-finalizar-consulta`);p&&(p.disabled=!0,p.innerHTML=`<span class="material-symbols-rounded spin">sync</span> Guardando...`);try{let{data:r,error:s}=await t.from(`consultas`).insert([{organizacion_id:o.perfilMedico.organizacionId,sucursal_id:o.perfilMedico.sucursalId,paciente_id:o.pacienteActual.id,medico_id:o.perfilMedico.id,finalizada:!0,fecha_cierre:new Date().toISOString(),peso_kg:c,temperatura:l,frecuencia_cardiaca:u,frecuencia_respiratoria:d,estado_mucosas:document.getElementById(`in-mucosas`).value,tiempo_llenado_capilar:document.getElementById(`in-tllc`).value,escala_dolor:f,motivo_consulta:document.getElementById(`in-anamnesis`).value,anamnesis:document.getElementById(`in-anamnesis`).value,diagnostico_presuntivo:document.getElementById(`in-diagnostico`).value,plan_tratamiento:document.getElementById(`in-indicaciones-receta`)?.value||`Sin indicaciones extras.`}]).select(`id`).single();if(s)throw s;if(i.length>0){let e=i.map(e=>({consulta_id:r.id,...e})),{error:n}=await t.from(`recetas_items`).insert(e);n&&console.warn(`Aviso: No se pudo guardar el detalle de la receta en DB`,n)}if(sessionStorage.setItem(`cargosConsultaPendiente`,JSON.stringify([])),sessionStorage.setItem(`pacienteCheckout`,JSON.stringify({nombre:o.pacienteActual?.nombre||`Desconocido`,tutor:document.getElementById(`txt-dueno-paciente`)?.textContent||o.pacienteActual?.clientes?.nombre_completo||`Sin Tutor asignado`,cliente_id:o.pacienteActual?.cliente_id||null})),(i.length>0||document.getElementById(`in-indicaciones-receta`)?.value)&&await n(`Consulta Guardada`,`Consulta guardada exitosamente.

¿Desea imprimir la receta médica ahora?`)){let e=o.perfilMedico.clinica,t=o.perfilMedico.sucursal,n=o.pacienteActual;a({logo_url:e.logo_url||``,nombre_clinica:t.nombre_sucursal||e.nombre_legal||`Clínica Veterinaria`,direccion:t.direccion||``,telefono:t.telefono_recepcion||``,correo:``,rfc:e.rfc_fiscal||``,folio_receta:`RX-${r.id.toString().slice(0,6).toUpperCase()}`,fecha_emision:new Date().toLocaleDateString(`es-MX`),nombre_veterinario:o.perfilMedico?.nombre||`Desconocido`,cedula_profesional:o.perfilMedico?.cedula||`En trámite`,especialidad:`Medicina Veterinaria General`,firma_url:o.perfilMedico.firma_url||``,propietario_nombre:document.getElementById(`txt-dueno-paciente`)?.textContent||n.clientes?.nombre_completo||`Sin nombre`,propietario_telefono:n.clientes?.telefono||``,propietario_email:n?.clientes?.email||``,paciente_nombre:n?.nombre||`Desconocido`,expediente_id:`EXP-${n?.id?.toString().slice(0,6).toUpperCase()||`XXX`}`,especie:n.especie,raza:n.raza,sexo:n.sexo,edad:(e=>{if(!e)return`Desconocida`;let t=new Date,n=new Date(e),r=t.getFullYear()-n.getFullYear(),i=t.getMonth()-n.getMonth();return(i<0||i===0&&t.getDate()<n.getDate())&&r--,r>0?`${r} años`:`Menor de 1 año`})(n.fecha_nacimiento),peso:`${c} kg`,senas_particulares:``,motivo_consulta:document.getElementById(`in-anamnesis`).value,diagnostico:document.getElementById(`in-diagnostico`).value,medicamentos:i.map(e=>({nombre:e.nombre_manual,concentracion:`-`,dosis:e.via_admin,via:`-`,frecuencia:e.frecuencia,duracion:`-`,cantidad:`-`})),observaciones:document.getElementById(`in-indicaciones-receta`)?.value?document.getElementById(`in-indicaciones-receta`).value.split(`\\n`).filter(e=>e.trim()):[],fecha_revision:``,hora_revision:``,motivo_revision:``})}let p=sessionStorage.getItem(`idCitaActiva`);if(p){let{error:e}=await t.from(`citas`).update({estado:`finalizada`}).eq(`id`,p);e&&console.warn(`No se pudo actualizar el estado de la cita a finalizada`,e),localStorage.removeItem(`borrador_consulta_`+p),sessionStorage.removeItem(`idCitaActiva`)}clearInterval(o.intervaloCronometro);let m=document.getElementById(`contenedor-cronometro`);m&&(m.hidden=!0),document.getElementById(`btn-limpiar-paciente`)?.click(),e.reset();let h=document.getElementById(`tbody-receta-farmacos`);h&&(h.innerHTML=``),o.pacienteActual=null,window.cargarModulo?window.cargarModulo(`CHECKOUT_FINAL_COBRO`):alert(`✅ Consulta guardada correctamente. Proceda a la caja.`)}catch(e){console.error(`[TRANSACTION ERROR] Fallo al guardar ECOP:`,e),alert(`Error al guardar en base de datos: ${e.message||`Verifica tu conexión y los datos ingresados.`}`)}finally{p&&(p.disabled=!1,p.textContent=`Firmar y Finalizar Consulta`)}})}export{c as inicializarConsultaIntegral};