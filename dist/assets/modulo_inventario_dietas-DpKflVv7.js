import{i as e,n as t}from"./conexion-DiH-JvMT.js";var n=null;async function r(){console.log(`[DIETAS] Inicializando catalogo de dietas de prescripcion...`);try{let r=await e();if(!r)throw Error(`Sin sesion activa`);let{data:i,error:a}=await t.from(`perfiles`).select(`id, organizacion_id, sucursal_id, rol`).eq(`id`,r.id).single();if(a||!i)throw Error(`Perfil no encontrado`);n=i}catch(e){console.error(`[DIETAS] Error critico de identidad:`,e.message);return}o(),l(),s(),f(),i()}async function i(){let e=document.getElementById(`tbody-dietas`);if(e){e.innerHTML=`<tr><td colspan="7" class="texto-centro">Cargando dietas...</td></tr>`;try{if(!n?.organizacion_id){e.innerHTML=`<tr><td colspan="7" class="texto-centro texto-peligro">Error: No se pudo identificar tu clinica. Recarga la pagina.</td></tr>`;return}let{data:r,error:i}=await t.from(`inventario_productos`).select(`*`).eq(`organizacion_id`,n.organizacion_id).eq(`categoria`,`dietas`).order(`created_at`,{ascending:!1});if(i)throw i;if(!r||r.length===0){e.innerHTML=`<tr><td colspan="7" class="texto-centro" style="color: var(--text-muted);">No hay dietas registradas.</td></tr>`;return}e.innerHTML=``,r.forEach(t=>{let n=document.createElement(`tr`),r=t.nombre_comercial||`Dieta sin nombre`;t.precio_venta&&`${parseFloat(t.precio_venta).toFixed(2)}`;let i=parseFloat(t.stock_total)||0,o=parseFloat(t.stock_minimo)||5,s=t.codigo_barras||`SIN-SKU`,c=t.metadata?.marca||`N/A`,l=t.metadata?.subcategoria||`otra`,u=a(l),d=t.metadata?.especie||`N/A`,f=t.metadata?.peso&&t.metadata?.unidad_peso?`${t.unidad_medida||`Bolsa`} ${t.metadata.peso} ${t.metadata.unidad_peso}`:t.unidad_medida||`N/A`,p=``;p=i>o?`<span class="badge-estado optimo" style="font-weight:bold; padding:4px 8px; border-radius:4px; background:#D1FAE5; color:#10B981;">Óptimo</span>`:i>0?`<span class="badge-estado critico" style="font-weight:bold; padding:4px 8px; border-radius:4px; background:#FEF3C7; color:#F59E0B;">Agotándose</span>`:`<span class="badge-estado agotado" style="font-weight:bold; padding:4px 8px; border-radius:4px; background:#FEE2E2; color:#EF4444;">Agotado</span>`,n.innerHTML=`
                <td>
