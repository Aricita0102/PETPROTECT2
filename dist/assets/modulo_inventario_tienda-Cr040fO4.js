        ...
        <div class="notif-footer">
            ...
            <button type="submit" form="form-nuevo-tienda-bento" class="notif-footer-btn primario" style="background:#F27405; display:flex; align-items:center; gap:6px; padding:10px 22px;">
                <span class="material-symbols-rounded" style="font-size:18px;">save</span> Guardar
                    ...
                    `)}),S.dispatchEvent(new Event(`change`)));let w=document.getElementById(`tienda-costo-bento-edit`),T=document.getElementById(`tienda-precio-bento-edit`),E=document.getElementById(`tienda-precio-base-bento-edit`),D=document.getElementById(`tienda-aplica-iva-bento-edit`),O=document.getElementById(`tienda-label-iva-bento-edit`),k=e=>{let t=D?D.checked:!0,n=t?1.16:1;if(O&&(O.innerHTML=t?`P. Final (16% IVA inc.) <span style="color:#F27405;">*</span>`:`P. Final (Sin IVA) <span style="color:#F27405;">*</span>`),e===`base`)T.value=((parseFloat(E.value)||0)*n).toFixed(2);else if(e===`final`)E.value=((parseFloat(T.value)||0)/n).toFixed(2);else if(e===`toggle`){let e=parseFloat(E.value)||0;T.dataset.modificado===`true`&&E.dataset.modificado!==`true`?E.value=((parseFloat(T.value)||0)/n).toFixed(2):T.value=(e*n).toFixed(2)}};D&&D.addEventListener(`change`,()=>k(`toggle`)),E&&T&&(T.value&&(T.dataset.modificado=`true`),E.addEventListener(`input`,e=>{e.isTrusted&&(E.dataset.modificado=`true`),k(`base`)}),T.addEventListener(`input`,e=>{e.isTrusted&&(T.dataset.modificado=`true`),k(`final`)}));let A=document.createElement(`div`);A.style=`font-size: 11px; color: #6b7280; margin-top: 4px;`,T.parentNode.appendChild(A);let j=()=>{let e=parseFloat(w.value),t=S.value;if(isNaN(e)||e<=0){A.innerHTML=``;return}let n=.3;t===`alimento`?n=.2:t===`accesorios`?n=.5:t===`juguetes`?n=.45:t===`higiene`&&(n=.35);let r=e/(1-n),i=Math.round(r*1.16),a=(i/1.16-e)/e*100;A.innerHTML=`Sugerido: <strong>$${i.toFixed(2)} MXN</strong> (Margen: ${n*100}%, ROI: ${a.toFixed(1)}%, IVA 16% inc.) 
