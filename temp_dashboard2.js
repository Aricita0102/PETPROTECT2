...
export async function inicializarModuloDashboard() {
    try {
        ...
        // Ô£à OPTIMIZACI├ôN: Singleton ÔÇö cero peticiones de red adicionales
        const sesion = await obtenerSesionActiva();
        if (!sesion) {
            console.error("ERROR CR├ìTICO DE INICIALIZACI├ôN: No hay sesi├│n activa.");
        ...
        // 5. Renderizado de UI y Validaci├│n de acceso
        actualizarPerfilUI(sesion.perfil);
        gestionarAccesoVeterinario(sesion.perfil);

        // 6. Conectar bot├│n cerrar sesi├│n
        ...
        }, 100); // Peque├▒o delay para asegurar que el DOM de la vista ya fue inyectado

    } catch (error) {
        console.error("ERROR CR├ìTICO DE INICIALIZACI├ôN:", error);
        alert("Error de consola detectado. Revisa con F12.");
    }
}
...
function gestionarAccesoVeterinario(perfil) {
    ...
    if (perfil.estatus === "suspendido") {
        alert("Ôøö Cuenta suspendida.");
        cerrarSesionApp();
        return;
    }

    // --- ├ëXITO: MOSTRAR CONTENIDO Y QUITAR LOADER ---
    if (contenidoPrincipal) contenidoPrincipal.style.display = "flex";
    if (loader) loader.style.display = "none"; // ­ƒæê Aqu├¡ se oculta al confirmar acceso
    ...
    // requestAnimationFrame garantiza que el DOM ya est├í pintado
...
// ÔöÇÔöÇÔöÇ Reloj y Horario (Widget) REMOVIDO POR ROLLBACK ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ

// ÔöÇÔöÇÔöÇ Buscador Predictivo ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
function configurarBuscadorPredictivo() {
    ...
    async function realizarBusqueda(termino) {
        ...
        try {
            ...
            dropdown.innerHTML = '<div class="resultado-pred-empty" style="color:red;">Error de b├║squeda</div>';
        }
    }
    ...
    function renderizarResultadosPredictivos(resultados) {
        ...
        resultados.forEach(paciente => {
            const fotoUrl = paciente.foto_url || 'https://cdn-icons-png.flaticon.com/512/2809/2809865.png'; // Fallback gen├®rico
            const nombreMascota = paciente.nombre || 'Desconocido';
            const especieRaza = `${paciente.especie || 'Mascota'} ${paciente.raza ? '┬À ' + paciente.raza : ''}`;
...
// ÔöÇÔöÇÔöÇ Bot├│n Nuevo Ingreso ÔåÆ Modal de Registro de Paciente ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
...
// ÔöÇÔöÇÔöÇ M├®tricas Casos Cl├¡nicos ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
async function cargarMetricasCasosClinicos() {
    ...
    try {
        ...
        console.log('[DASH] ÔåÆ Casos cl├¡nicos, org:', organizacionIdActual, '| desde:', inicioMes);

        // Trae TODAS las consultas del mes con sus campos m├¡nimos.
        // RLS en Supabase deja pasar las filas donde el usuario es medico_id
        // O donde organizacion_id coincide si las pol├¡ticas son por org.
        ...
        console.log('[DASH] ÔåÉ Casos del mes:', total, '| Finalizadas:', finalizadas, '| Pacientes:', pacientesUnicos);
        ...
        // ÔöÇÔöÇÔöÇ Renderizado de Gr├ífica de Dona (Casos) ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
        ...
        if (ctxCasos && window.Chart) {
            // Control de SPA: Destrucci├│n previa
            ...
            window._instanciaGraficaCasos = new Chart(ctxCasos, {
                type: 'doughnut',
                data: {
                    labels: ['Finalizadas', 'En Revisi├│n'],
                    datasets: [{
        ...
        console.error('[DASH] Ô£ÿ M├®tricas cl├¡nicas fallaron:', err.message);
        if (elTotal) elTotal.textContent = '!';
        if (elFinal) elFinal.textContent = 'ÔÇö';
        if (elAbiertas) elAbiertas.textContent = 'ÔÇö';
        if (elPacientes) elPacientes.textContent = 'ÔÇö';
    }
}

// ÔöÇÔöÇÔöÇ M├®tricas de Ingresos (Caja Transacciones) ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
async function cargarMetricasIngresos() {
    ...
    try {
        ...
        console.log('[DASH] ÔåÆ Ingresos Reales de Caja, org:', organizacionIdActual);
        ...
        if (error) {
            // Si la tabla a├║n no existe, fallar├í aqu├¡ sin romper el resto del dashboard
            if (error.code === '42P01') {
                console.warn('[DASH] ÔÜá´©Å Tabla caja_transacciones a├║n no creada en Supabase. Corre el SQL.');
            } else {
                throw error;
            }
        }
        ...
        if (elTotal)  elTotal.textContent  = $fmt(totalMes).replace('$', ''); // N├║mero grande sin $
        ...
        console.log('[DASH] ÔåÉ Ingresos Totales Mes:', totalMes);

        // ÔöÇÔöÇÔöÇ Promesa de Carga Din├ímica de Chart.js ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
        const cargarLibreriaChartJS = () => new Promise((resolve) => {
            if (window.Chart) return resolve();
            console.log("[DASH] Inyectando Chart.js din├ímicamente...");
            const script = document.createElement('script');
            script.src = 'https://cdn.jsdelivr.net/npm/chart.js';
            script.onload = () => resolve();
            document.head.appendChild(script);
        ...
        // ÔöÇÔöÇÔöÇ Renderizado de Gr├ífica de Ingresos ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
        const ctxGrafica = document.getElementById('graficaIngresosMensuales');
        if (ctxGrafica && window.Chart) {
            // Destrucci├│n de instancia previa para SPA lifecycle
            if (window._instanciaGraficaIngresos) {
                window._instanciaGraficaIngresos.destroy();
            }

            // Agrupaci├│n Time-Series cronol├│gica
            ...
            window._instanciaGraficaIngresos = new Chart(ctxGrafica, {
                ...
                options: {
                    ...
                    scales: {
                        y: {
                            beginAtZero: true,
                            grid: { display: false }, // Ocultar cuadr├¡cula Y
                            ticks: {
                                font: { family: 'Inter' },
                                color: '#64748b',
                        ...
                        x: {
                            grid: { display: false }, // Ocultar cuadr├¡cula X
                            ticks: {
                                font: { family: 'Inter' },
                                color: '#64748b'
                            }
        ...
        console.error('[DASH] Ô£ÿ M├®tricas ingresos fallaron:', err.message);
        if (elTotal) elTotal.textContent = '!';
        [elMes, elSemana, elHoy].forEach(el => { if (el) el.textContent = '$ÔÇö'; });
    }
}
...
function actualizarPerfilUI(datos) {
    ...
    if (tituloBienvenida) {
        ...
        tituloBienvenida.textContent = `Bienvenido(a) ${primerNombre}, ┬ías├¡ est├ín las cosas hoy!`;
    }
}
...
function renderizarCitas(citas, contenedor, contador) {
    ...
    if (citas && citas.length > 0) {
        ...
        contenedor.innerHTML = `<div style="text-align: center; padding: 2rem;"><p>Sala vac├¡a.</p></div>`;
    }
...
function iniciarSistemaAlertas() {
    ...
    canalAlertasInventario
        .on('postgres_changes', { 
            ...
            // Si el nuevo stock total cruz├│ el umbral del m├¡nimo
...
async function cargarNotificacionesDashboard() {
    ...
    if (!errorInv && dataInv) {
        ...
        // Agregar cada producto cr├¡tico como una notificaci├│n
        stockCritico.forEach(p => {
            todasNotificaciones.push({
                tipo: 'alerta_stock',
                titulo: `Inventario Cr├¡tico (${p.categoria})`,
                mensaje: `El producto "${p.nombre_comercial}" est├í en nivel bajo o agotado (Stock: ${p.stock_total}).`,
    ...
    // Ordenar y limitar a las ├║ltimas 6
...
async function inicializarFirebasePush() {
    ...
    try {
        ...
        if (permission === 'granted') {
            ...
            // Reemplaza "TU_VAPID_KEY" con la llave VAPID de tu proyecto Firebase (pesta├▒a Cloud Messaging)
            ...
            try {
                ...
                if (token) {
                    ...
                    // Aqu├¡ deber├¡as guardar este token en Supabase en el perfil del usuario:
                    // await conexionSupabase.from('perfiles').update({ fcm_token: token }).eq('id', usuarioActualId);
                }
            } catch (err) {
                console.warn('No se pudo obtener el token VAPID. Faltan llaves v├ílidas:', err);
            }

            // Escuchar mensajes cuando la app est├í ABIERTA (Foreground)
...
function configurarBotonScanner() {
    ...
    btnCerrar.addEventListener('click', () => {
        ...
        // NO removemos el canal aqu├¡ para que siga escuchando en background
    });
    ...
    async function iniciarSesionScanner() {
        ...
        statusTxt.textContent = 'Esperando conexi├│n...';
        ...
        scannerChannel.on('broadcast', { event: 'scan' }, (payload) => {
            ...
            // Guardar c├│digo escaneado en variable global para que otros m├│dulos (como Caja Registradora) lo puedan consumir
        ...
        scannerChannel.subscribe((status) => {
            if (status === 'SUBSCRIBED') {
                ...
                statusTxt.textContent = 'Esc├íner conectado (Esperando lecturas...)';
